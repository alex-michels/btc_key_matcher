...
pub fn acquire_chunk(
    ...
    let entries = fs::read_dir(base_path).unwrap_or_else(|_| panic!("Failed to read {}", base_path));
        ...
        if let Ok(meta) = fs::read_to_string(&path) {
            if let Ok(mut chunk) = serde_json::from_str::<ChunkMetadata>(&meta) {
                if chunk.status == ChunkStatus::Pending {
                    let id = BigUint::parse_bytes(chunk.chunk_id.trim_start_matches('0').as_bytes(), 10)
